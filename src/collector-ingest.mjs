...
const ADMISSION_MIN_GAP_MS=Number(process.env.COLLECTOR_OI_MIN_GAP_MS||60000);
...
export async function ingestCollectorSnapshot(snapshot,{signature,rawBody}={}){
 ...
 const prior=(await readEvidence('futures_state',{limit:200})).map(x=>x.payload||x).find(x=>x.symbol===snapshot.symbol&&x.source===snapshot.source&&Number.isFinite(Number(x.openInterest))&&x.sourceObservedAt!==snapshot.sourceObservedAt);const prevOi=prior?Number(prior.openInterest):null,oi=Number(snapshot.openInterest),gapMs=prior?Date.parse(snapshot.sourceObservedAt)-Date.parse(prior.sourceObservedAt):null,independent=Number.isFinite(gapMs)&&gapMs>=ADMISSION_MIN_GAP_MS,deltaOiPct=independent&&Number.isFinite(prevOi)&&prevOi!==0?(oi-prevOi)/prevOi*100:null;
 const row={...snapshot,previousOpenInterest:prevOi,previousSourceObservedAt:prior?.sourceObservedAt||null,observationGapMs:gapMs,independentObservation:independent,deltaOiPct,ageMs:q.ageMs,completeness:q.completeness,quality:'VERIFIED',transport:'signed-external-collector',sensorAdmission:deltaOiPct===null?'WAITING_FOR_INDEPENDENT_OI1':'ELIGIBLE_FOR_SHADOW_TRIAL'};
