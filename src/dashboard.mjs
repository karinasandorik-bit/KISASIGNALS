...
function product(){const rows=dbRows??read(LEDGER),settlements=dbSettlements??read(SETTLEMENTS),byPrediction=new Map(settlements.map(x=>{const s=x.payload||x;return [s.predictionId,s]}));const view=x=>{const r=x.payload||x,id=r.predictionId||r.prediction_id||r.decision_id||r.id;return signalProductView(r,futuresState,byPrediction.get(id)||null,candles)};return {signals:rows.map(view),openEntries:rows.map(view).filter(x=>['WAITING','IN_ENTRY','ACTIVE'].includes(x.status)&&x.action!=='NO_TRADE'),statistics:evidenceStats({rows,settlements,trials:dbTrials,trialSettlements:dbTrialSettlements})}}
