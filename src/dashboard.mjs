...
let latest=null,lastError=null,running=false,dbHealth={backend:'initializing',postgres:false},dbRows=null,dbSettlements=null;
...
function payload(){return JSON.stringify({latest,rows:dbRows??read(LEDGER),settlements:dbSettlements??read(SETTLEMENTS),persistence:dbHealth,lastError,server_time:new Date().toISOString(),refresh_ms:REFRESH_MS})}
...
async function tick(){if(running)return;running=true;try{const m=await market();latest=await runLive({marketFn:async()=>m,ledger:LEDGER});await settleEligible({predictions:LEDGER,settlements:SETTLEMENTS,bars:m.bars,now:m.received_at});await refreshEvidence();lastError=null}catch(e){lastError={at:new Date().toISOString(),message:e.message}}finally{running=false;broadcast()}}
