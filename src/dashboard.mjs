...
function payload(){return JSON.stringify({latest,rows:read(LEDGER),settlements:read(SETTLEMENTS),lastError,server_time:new Date().toISOString(),refresh_ms:REFRESH_MS})}
function broadcast(){const d='data: '+payload()+'\n\n';for(const r of clients)r.write(d)}
async function tick(){if(running)return;running=true;try{const m=await market();latest=await runLive({marketFn:async()=>m,ledger:LEDGER});settleEligible({predictions:LEDGER,settlements:SETTLEMENTS,bars:m.bars,now:m.received_at});lastError=null}catch(e){lastError={at:new Date().toISOString(),message:e.message}}finally{running=false;broadcast()}}
const html=fs.readFileSync(new URL('../public/index.html',import.meta.url),'utf8');
const server=http.createServer((req,res)=>{if(req.url==='/api/state'){res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});return res.end(payload())}if(req.url==='/events'){res.writeHead(200,{'content-type':'text/event-stream','cache-control':'no-cache','connection':'keep-alive'});clients.add(res);res.write('data: '+payload()+'\n\n');req.on('close',()=>clients.delete(res));return}if(req.url==='/health'){res.writeHead(lastError?503:200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:!lastError,lastError}))}res.writeHead(200,{'content-type':'text/html; charset=utf-8','cache-control':'no-store'});res.end(html)});
