...
function product(){const rows=dbRows??read(LEDGER),settlements=dbSettlements??read(SETTLEMENTS);return {signals:rows.map(x=>signalProductView(x.payload||x)),openEntries:openEntries(rows.map(x=>x.payload||x)),statistics:evidenceStats({rows,settlements,trials:dbTrials,trialSettlements:dbTrialSettlements})}}
