...
export const EXPECTED_MODEL_SHA256='29f643ecf0bf98dba652aa939f22400c530592418d80b3d6e670c128212c0dda'; // replaced below from actual frozen bytes
...
export function inferPriceNet(bars){const{artifact,sha256}=loadPriceNet(),m=artifact.model,x0=featureVector(bars),x=x0.map((v,j)=>(v-m.featureMean[j])/m.featureStd[j]);const logits=m.weights.map((w,k)=>w.reduce((s,v,j)=>s+v*x[j],m.bias[k]));const p=softmax(logits),entropy=-p.reduce((s,v)=>s+(v?v*Math.log(v):0),0)/Math.log(3),ood=Math.min(1,Math.sqrt(mean(x.map(v=>v*v)))/4);return{modelVersion:m.version,modelSha256:sha256,features:x0,pUp:p[0],pDown:p[1],pRange:p[2],uncertainty:entropy,ood,horizonBars:m.horizonBars,thresholdBps:m.thresholdBps}}
