...
test('legacy excursion heads are audit-only point estimates',()=>{const p=inferPriceNet(bars());assert.equal(p.excursionStatus,'POINT_ESTIMATE_ONLY');assert.equal(p.calibrationStatus,'UNVERIFIED');assert.ok(Number.isFinite(p.legacyExcursion.mfePointBps));assert.ok(Number.isFinite(p.legacyExcursion.maePointBps))});
